    ...
    
{
  ...
  */
  ...
  framePtrY0  = frameY0 >> 2;				/* (s15.0f) */
  frameRemain = frameH  >> 2; 				/* (s15.0f) */
...

{
  ...
  */
  ...
  tmemSize  = TMEMSIZE[bg->s.imageFmt];		/* (s15.0f) */
  tmemMask  = TMEMMASK[bg->s.imageSiz];		/* (s10.5) */
  tmemShift = TMEMSHIFT[bg->s.imageSiz];	/* (s10.5)->(s15.0f) */
...
/* addition 99/05/31(Y) */
  ...
  /* Get TMEM/image/frame line number that can be loaded at once */
   tmemSliceLines = tmemSize / tmemSliceWmax;		/* (s15.0f) */
  imageSliceLines = tmemSliceLines - flagBilerp;	/* (s15.0f) */
  ...
  imageTop      = (u32)bg->s.imagePtr;			/* (u32.0f) */
  imageSrcWsize = (imageSrcW / tmemShift) << 3;		/* (u16.0f) */
  imagePtrX0    = (imageX0   / tmemShift) << 3;		/* (u16.0f) */
  ...
  
{
  ...
  while(1){
	...
	if (imageSliceH > imageSliceLines) imageSliceH = imageSliceLines;
      ...
      framePtrY1 = framePtrY0 + frameSliceH;		/* (s15.0f) */
